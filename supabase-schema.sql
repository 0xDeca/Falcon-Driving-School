...
-- Driving licenses (student uploads for admin verification)
...
CREATE POLICY "Students can view own license" ON driving_licenses
  FOR SELECT USING (EXISTS (SELECT 1 FROM students WHERE id = driving_licenses.student_id AND user_id = auth.uid()));
CREATE POLICY "Students can insert own license" ON driving_licenses
  FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM students WHERE id = driving_licenses.student_id AND user_id = auth.uid()));
